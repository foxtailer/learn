...
def main(request):
    ...
    print(script_dir + "/media/plot.png")
    ...
    def mv(df, etflist = ['BNDX', 'SPSM', 'SPMD', 'SPLG', 'VWO', 'VEA', 'MUB', 'EMB'], short = 0, maxuse = 1, normal = 1, startdate = 199302, enddate = 202312):
        ...
        try: 
            ...
            if normal: 
                ...
                # Just save without drawing.
                plt.savefig("/home/zoy/vscode/learn/projects/scripts/csv_web_plot/dj_csv_web_plot/media/plot.png")
    ...
    if request.method == "POST":
        etf = request.FILES.get('file_ETF')
        ff = request.FILES.get('file_F-F')
        
        etf_path = os.path.join(settings.MEDIA_ROOT, 'file_ETF.csv')
        ff_path = os.path.join(settings.MEDIA_ROOT, 'file_F-F.csv')
        # Save the 'etf' file
        with open(etf_path, 'wb+') as destination:
            for chunk in etf.chunks():
                destination.write(chunk)

        # Save the 'ff' file
        with open(ff_path, 'wb+') as destination:
            for chunk in ff.chunks():
                destination.write(chunk)

        if not etf or not ff:
            return JsonResponse({'error': 'Both file required.'}, status=400)

        # Create a file-like object from the uploaded file
        # etf_content = etf.read().decode()
        # etf_reader = io.StringIO(etf_content)

        # ff_content = ff.read().decode()
        # ff_reader = io.StringIO(ff_content)

      
        #df = get_and_merge("/home/zoy/Desktop/ETF_returns.csv", "/home/zoy/Desktop/F-F_Research_Data_Factors.csv")
        df = get_and_merge(ff_path, etf_path)
        #df = get_data(ff_reader)
        etflist = ['BNDX', 'SPSM', 'SPMD', 'SPLG', 'VWO', 'VEA', 'MUB', 'EMB']
        mv(df, etflist, 0, 1, 0, 201512, 202312)

        
        image_url = os.path.join(script_dir, "/media/plot.png")
        return render(request, 'main/main.html', {'image_url': image_url})
        ...
        #     file1_data = data.get('file1', None)
        #     file2_data = data.get('file2', None)
